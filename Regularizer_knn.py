...
class Regularizer_knn():
    def __init__(self, args, name, train_triples, nentity, nrelation):
        ...
        self.knn = defaultdict(dict)
        self.knn_size = defaultdict(dict)
        self.MAX_K = 30
        self.load_cached_knn = True
        self.build_knn()
    ...
    def dist(self, f1, f2): # compute distance between two entities
    ...
    def fill_zeros(self): # pad for diffeerent numbers of connected(h), defined in Eq (24) of Section 4.3
        for mode in self.knn:
            for t1 in self.knn[mode]:
                while len(self.knn[mode][t1]) < self.n_pos:
                    self.knn[mode][t1].append((0,0,0))

    def load_pickle(self): # load existing connected(h), defined in Eq (24) of Section 4.3
        if self.dist_func == "jaccard":
            knn_file = self.name + '_knn.pkl'
            knn_size_file = self.name + '_knn_size.pkl'
        else:
            knn_file = self.name + '_knn_rand.pkl'
            knn_size_file = self.name + '_knn_size_rand.pkl'
        if os.path.exists(knn_file):
            print('load knn pickle')
            with open(knn_file, "rb") as f:
                self.knn = pickle.load(f)
            with open(knn_size_file, "rb") as f:
                self.knn_size = pickle.load(f)
            print('load knn pickle finished')
            print('loaded' + str([t for t in self.knn_size.keys()]))
    ...
    def dump_pickle(self): # save connected(h), defined in Eq (24) of Section 4.3
        if self.dist_func == "jaccard":
            ...
            knn_file = self.name + '_knn_rand.pkl'
            knn_size_file = self.name + '_knn_size_rand.pkl'
        print('dump knn pickle')
        with open(knn_file, "wb") as f:
            pickle.dump(self.knn, f)
        with open(knn_size_file, "wb") as f:
            pickle.dump(self.knn_size, f)
        print('dump knn pickle finished')

    def build_knn(self): # build connected(h), defined in Eq (24) of Section 4.3
        if self.load_cached_knn:
            self.load_pickle()
        modify_flag = False

        loaded_modes = [t for t in self.knn_size.keys()]
        for mode in loaded_modes:
            if mode.count('-') == 1:
                ...
                if mode_new not in self.knn:
                    self.knn[mode_new] = self.knn.pop(mode)
                modify_flag = True
        loaded_modes = [t for t in self.knn_size.keys()]
        ...
        for mode in self.mode_list:
            ...
            for triple in tqdm.tqdm(self.train_triple_list,desc='build features (knn) {}'.format(mode)):
            ...
            for t1 in tqdm.tqdm(tuple_features, desc = 'compute knn {}'.format(mode)): # [(1174,186,0),(2160,186,0),(4994,186+474//2,0)]:
                neighbors = set()
                ...
                for emb_mode in ['hrt', 'hr', 'h', 'rt', 't']:
                    ...
                    if mode_this in self.mode_list and mode_this not in loaded_modes:
                        ...
                        self.knn_size[mode_this][t1] = min(self.MAX_K,len(dist[mode_this]))
                        self.knn[mode_this][t1] = [t[0] for t in dist[mode_this][:self.knn_size[mode_this][t1]]]
                        while len(self.knn[mode_this][t1]) < self.MAX_K:
                            self.knn[mode_this][t1].append((0,0,0))
        if modify_flag and self.load_cached_knn:
            self.dump_pickle()

        self.fill_zeros()

    def point_plane_dis_seperate_group(self, x, vecs, sz): # bs * 1 * dim,  bs * n_pos' * dim
    ...
    def get_knn_emb(self, x, embeddings, source_mode, feature_mode, emb_mode): # get positive and negative embedding for a batch
        ...
        for h, r, t in nx:
            ...
            s = min(self.knn_size[mode][t1],self.n_pos)
            if self.knn_size[mode][t1] >= self.MAX_K:
                triples = empty_triples
                idx.append(triples)
                size.append(0)
            else:
                triples = random.sample(self.knn[mode][t1][:self.knn_size[mode][t1]],s)
    ...
    def forward(self, x, embeddings, model = None): # compute \hat{cr} defined in Eq (25) of Sec 4.5
        dis_func = self.point_plane_dis_seperate_group
        ...
        for i,(mode,weight) in enumerate(zip(self.mode_list,self.weight_list)):
            ...
            pos_emb_h, pos_emb_r, pos_emb_t, tuples, pos_size = self.get_knn_emb(x, embeddings, source_mode, feature_mode, emb_mode)
